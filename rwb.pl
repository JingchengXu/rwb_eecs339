...
if ($action eq "login") {
  ...
  if ($run) { 
    ...
    if($user eq "root"){
      ...
      $confirm_psw=crypt($password, $salt);
    }
    if (ValidUser($user,$confirm_psw)) { 
      ...
      $outputcookiecontent=join("/",$user,$confirm_psw);
      $action = "base";
      $run = 1;
    } else {
...
if ($action eq "base") { 
  ...
  my @total_cycle;
  eval{@total_cycle=ExecSQL($dbuser,$dbpasswd,"select distinct cycle from cs339.candidate_master");};
# print"<table id='Voting_summary'><tr><td><Voting Data: ></td><td id='voting_data'>(No Cycle Selected)</td></tr></table>";  
   if(UserCan($user,"query-opinion-data"))
  {
  print 
 "<form>
<input type=\"checkbox\" id=\"committee\" onclick='ViewShift()' >Committees</input><br><input type=\"checkbox\" onclick='ViewShift()' id=\"candidate\" >Candidates</input><br><input type=\"checkbox\" id=\"individual\" onclick='ViewShift()'>Individuals</input><br><input type=\"checkbox\" onclick='ViewShift()' id=\"opinion\" >Opinions</input><br></form>";
 

    print "You are permitted to query the opinion";
    print 
    hr,"\n";
  }
   if(!UserCan($user,"query-opinion-data"))
  ...
  print 
 "<form>
<input type=\"checkbox\" id=\"committee\" onclick='ViewShift()' >Committees</input><br><input type=\"checkbox\" onclick='ViewShift()' id=\"candidate\" >Candidates</input><br><input type=\"checkbox\" id=\"individual\" onclick='ViewShift()'>Individuals</input><br></form>";
  print "<input type=\"checkbox\" id=\"opinion\" onclick='ViewShift()' style=\"display:none\"></input>";

   print "You do not have the permission to query the opinion";
   print hr,"\n";
  }

 
  foreach(@total_cycle){
  print "<input type='checkbox' id=@{$_} class='cycles'onclick='ViewShift()'>@{$_}";
...
#Summary of Voting
#print "<table id='voting_summary'>";
#print "<tr><td>Voting Summary:  </td><td id='Winner'>(Not selected)</td></tr></table>";
...
# height=1024 width=1024 id=\"info\" name=\"info\" onload=\"UpdateMap()\"></iframe>";
  ...
  if ($user eq "anon") {
    ...
    if (UserCan($user,"manage-users") || UserCan($user,"invite-users")) {
      print "<p><a href=\"rwb.pl?act=invite-user&user=$user\">Invite User</a></p>";
    }
...
if ($action eq "near") {
 ...
 my $whatparam = param("what");
  ...
  my (@result,$error) = getWinner($cycle,$format);
     if (!$error) {
      if ($format eq "table") { 
	print "<h2>Winner Information</h2>@result";
      } else {
	print @result;
      }
    }   
 ...
 # my $voting_id='voting_data';
 # foreach $item(@total_cycle)
 # {
 # my $text+="<span>Year: $item\$ </span><span>Name: $temp_6\$</span><span> Party: $temp_7\$</span><span>Total Amount of Money in the Map View For the winner's Party: $temp_8\$</span><br>"; 
 # }
  if ($what{committees}) { 
...
my $counting;
  ...
  ($temp_1,$temp_2,$temp_3,$temp_4) = ($rows_time_rep_cand[0],$rows_time_dem_cand[0],$rows_time_rep_comm[0],$rows_time_dem_comm[0]);
 
 $counting= @{$temp_1}[0] + @{$temp_2}[0]+@{$temp_3}[0] + @{$temp_4}[0];
  ...
  if ($counting<20)
{
  ...
  my $temp_5=@{$temp_1}[1]+@{$temp_3}[1];
  my $temp_6=@{$temp_2}[1]+@{$temp_4}[1];
            if($temp_5>$temp_6)
        {
        my $color='red';
        my $id='committee_contribution';
        my $text = "<span>Republican Contributions: $temp_5\$ </span><span>Democrat Contributions: $temp_6\$</span>";
      ...
      my $temp_max_Dem=@{$temp_4}[2];
      my $temp_min_Dem=@{$temp_4}[3];
      my $temp_avg_Dem=@{$temp_4}[4];
      my $temp_max_Rep=@{$temp_3}[2];
      my $temp_min_Rep=@{$temp_3}[3];
      my $temp_avg_Rep=@{$temp_3}[4];

 	my $color='red';
        my $id='committee_data_contribution_1';
        my $text = "<span>MAX: $temp_max_Dem\$</span> <span>MIN: $temp_min_Dem\$</span> <span>AVG: $temp_avg_Dem\$</span>";
        print "<div id='$id' color='$color' style='display:none;'>$text</div>";
        
	my $color='blue';
        my $id='committee_data_contribution_2';
        my $text = "<span>MAX: $temp_max_Rep\$</span> <span>MIN: $temp_min_Rep\$</span> <span>AVG: $temp_avg_Rep\$</span>";
        print "<div id='$id' color='$color' style='display:none;'>$text</div>";
  ...
  if ($what{individuals}) {
    ...
    eval{ 
        @rows_time_dem = ExecSQL($dbuser, $dbpasswd, "select count(transaction_amnt),sum(transaction_amnt) from (cs339.individual natural join cs339.ind_to_geo)  where  cycle in ($cycle) and latitude>? and latitude<? and longitude>? and longitude<? and CMTE_ID in (select cmte_id from cs339.committee_master where cmte_pty_affiliation='Dem' or cmte_pty_affiliation='DEM')",undef,$latsw,$latne,$longsw,$longne);
  }; 
    eval { 
   @rows_time_rep = ExecSQL($dbuser, $dbpasswd, "select count(transaction_amnt),sum(transaction_amnt)from (cs339.individual natural join cs339.ind_to_geo)  where  cycle in ($cycle) and latitude>? and latitude<? and longitude>? and longitude<? and CMTE_ID in (select cmte_id from cs339.committee_master where cmte_pty_affiliation='Rep' or cmte_pty_affiliation='REP')",undef,$latsw,$latne,$longsw,$longne);
  }; 
 ($temp_1,$temp_2) = ($rows_time_rep[0],$rows_time_dem[0]); 
 $counting= @{$temp_1}[0] + @{$temp_2}[0];
  if ($counting<20)
{
   $latne+=0.1;
   $latsw-=0.1;
...
eval {
  ...
  ($temp_1,$temp_2) = ($rows_time_rep[0],$rows_time_dem[0]);   
   ...
   my $temp_4=@{$temp_2}[1];
        ...
        {
        my $color='red';
        my $id='Individual_contribution';
        my $text = "<span>Republican Contributions: $temp_3\$ </span><span>Democrat Contributions: $temp_4\$</span>";
      	print "<div id='$id' color='$color' style='display:none;'>$text</div>";
        }
        if($temp_3<$temp_4)
        {
...
@rows = ExecSQL($dbuser, $dbpasswd, "select avg(color), stddev(color), count(color) from rwb_opinions where latitude>? and latitude<? and longitude>? and longitude<?","COL",$latsw,$latne,$longsw,$longne);
    ...
    if($temp_1>0)
        ...
        my $text = "<span>Average is: $temp_1\$ </span><span>Standard Deviation is: $temp_2\$</span>";
      	print "<div id='$id' color='$color' style='display:none;'>$text</div>";
        }
        if($temp_1<0)
        {
...
if ($action eq "invite-user") { 
  print h2("It is the web for inviting user");
  ...
  print h2('you do not have the permission to invite users.');
   ...
   # print '1';
    #print $link;
   # print @total_action;
    #foreach(@total_action)
    #{
    #print @{$_};
    #}

    my @total_action;
    eval {@total_action=ExecSQL($dbuser,$dbpasswd,"select distinct action from rwb_permissions where rwb_permissions.name=?","COL",$user);};  
 #  print @total_action;
    print start_form(-name=>'InviteUser'),
    h2('Invite User'),
    "Email: ", textfield(-name=>'email'),
    p,
    checkbox_group(-name=>'action_box', -values=>[@total_action],-id=>[@total_action]),
    hidden(-name=>'run',-default=>['1']),
  ...
  print "$email";
  @selectedAction=param('action_box');
  my $temp=join(',',@selectedAction);
  print $temp;
  my $user=param("user");
  my $time=time();
  my $used=0;

  my $link="http://murphy.wot.eecs.northwestern.edu/~jxb687/rwb_1/rwb.pl?act=adding&right=$temp&user=$user&time=$time";
 eval {ExecSQL($dbuser,$dbpasswd,"insert into uuid(id,used) values(?,?)",undef,$time,$used)};
...
if ($action eq "adding") { 
 ...
 #  print $time;
   my @rand;
    eval {@rand=ExecSQL($dbuser,$dbpasswd,"select used  from uuid  where uuid.id=?","COL",$time);}; 
   my $used=@rand[0];
 #  print $used;
   if($used==0)
   {
   if (!$run) { 
	...
	h2('Add User'),
	  "Name: ", textfield(-name=>'name'),
		...
		p,
		  "Password: ", password_field(-name=>'password'),
		    p,
		      hidden(-name=>'run',-default=>['1']),
			hidden(-name=>'act',-default=>['adding']),
			  hidden(-name=>'user',default=>$user),
                           hidden(-name=>'right',default=>$right),
                            hidden(-name=>'time',default=>$time),
                            submit,
			    end_form,
			      hr;
    } else {
      ...
      my $encrypted_psw=crypt($password, $salt);
      my $user=param('user');
      my $right=param('right');
      my $error;
      my $item;
      my $time=param('time');
      eval {ExecSQL($dbuser,$dbpasswd,"update uuid set used=1 where uuid.id=?",undef,$time);};
  #    print $time;
      my @seperate=split(/,/,$right);
      $error=UserAdd($name,$encrypted_psw,$email,$user);            
      foreach $item(@seperate)
      {
      GiveUserPerm($name,$item);
       }     
   ...
   print 'The link has been outdated.';
   }
#  my $name=param('name');
#  print "<p><a href=\"rwb.pl?act=base&run=1\">Return</a></p>";
}
...
if ($action eq "give-opinion-data") { 
 # print h2("Giving Location Opinion Data Is Unimplemented");
   ...
   if (!UserCan($user,"give-opinion-data")) { 
    ...
    h2('Give Opinion'),
    "Please select your opinion: ", p,
    radio_group(-name=>'action_box_opinion', -values=>[@total_opinion],-id=>[@total_opinion]),
    hidden(-name=>'run',-default=>['1']),
    ...
    my $lat;
    my $lng; 
    print $inputlocationcookiecontent;
  ...
  #  print $inputlocationcookiecontent;
    ...
    # print "$lat,$lng";
      ...
      my $name=param('user');
      my $opinion=param('action_box_opinion');
      my $error;
      
      $error=GiveOpinion($name,$opinion,$lat,$lng);        
      if ($error) { 
 #       print "See $lng,$lat";
	print "Can't Give Opinion because of $error;"
      } else {
	print "Opinion is given by $name which is $opinion .At($lat,$lng)";
      }                      
    }
  }
  print "<p><a href=\"rwb.pl?act=base&run=1\">Return</a></p>";
...
$format = "table";
my @rows;
eval { @rows = ExecSQL($dbuser, $dbpasswd, "select * from voting_data where year in ($cycle)", undef);};
if ($@) { 
    return (undef,$@);
  } else {
    if ($format eq "table") { 
     return (MakeTable("winner","2D",
["year", "name", "party"],
@rows),$@);
    } else {
      return (MakeRaw("winner","2D",@rows),$@);
    }
  }
}
...
sub Committees {
  ...
  my @rows;
  my $sql = "select latitude, longitude, cmte_nm, cmte_pty_affiliation, cmte_st1, cmte_st2, cmte_city, cmte_st, cmte_zip from cs339.committee_master natural join cs339.cmte_id_to_geo where cycle in ($cycle) and latitude>? and latitude<? and longitude>? and longitude<?";
  eval {
    @rows = ExecSQL($dbuser, $dbpasswd, $sql, undef,$latsw,$latne,$longsw,$longne);
  };
...
sub Candidates {
  my ($latne,$longne,$latsw,$longsw,$cycle,$format) = @_;
  my @rows;
  my $sql = "select latitude, longitude, cand_name, cand_pty_affiliation, cand_st1, cand_st2, cand_city, cand_st, cand_zip from cs339.candidate_master natural join cs339.cand_id_to_geo where cycle in ($cycle) and latitude>? and latitude<? and longitude>? and longitude<?";
eval {
  @rows = ExecSQL($dbuser, $dbpasswd, $sql, undef,$latsw,$latne,$longsw,$longne);
};
...
sub Individuals {
  my ($latne,$longne,$latsw,$longsw,$cycle,$format) = @_;
  my @rows;
  my $sql = "select latitude, longitude, name, city, state, zip_code, employer, transaction_amnt from cs339.individual natural join cs339.ind_to_geo where cycle in ($cycle) and latitude>? and latitude<? and longitude>? and longitude<?";
  eval {
    @rows = ExecSQL($dbuser, $dbpasswd, $sql,undef,$latsw,$latne,$longsw,$longne);
  };
...
# 
#GiveOpinion($name,$color,$latitude,$longtitude)

#sub Insert_time{
#eval {ExecSQL($dbuser,$dbpasswd,
#		 "insert into one_time_used (id,used) values (?,?)",undef,@_);};
 # return $@;
#}
